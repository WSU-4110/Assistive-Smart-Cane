...
export default function Scan() {
  ...
  function handleConnect() {
    ...
    alert(`Connecting to ${selectedId}`);
  }
      ...
      <main className="max-w-md mx-auto p-5">
        ...
        <Card className="p-6 md:p-8 mb-8">
          <div className="flex items-center justify-between gap-6">
            ...
            <Button size="lg" onClick={handleScan} disabled={scanning} className="rounded-full">
        ...
        <section aria-live="polite" aria-label="Available devices" className="mt-10">
              ...
              devices.map((d, i) => {
                ...
                return (
                  <button
                    key={d.id}
                    type="button"
                    className={`w-full text-left px-5 py-4 flex items-center justify-between gap-6 ${
                      i === devices.length - 1 ? "" : "border-b-2 border-border"
                    } ${selected ? "bg-muted" : "hover:bg-muted"}
                       focus:outline-none focus:ring-4 focus:ring-blue-300`}
                    onClick={() => setSelectedId(d.id)}
                    aria-pressed={selected}
                    aria-label={`Device ${d.name || "Unknown device"}${
                      selected ? " selected" : ""
                    }`}
                  >
                    <div className="min-w-0">
                    ...
                    <span className="text-sm">{selected ? "Selected" : "Tap to select"}</span>
                  </button>
                );
              })
            )}
        ...
        <Button
          variant="outline"
          size="lg"
          className="w-full rounded-full py-4 mt-10"
        ...
        <p className="mt-3 text-xs text-muted-foreground text-center pb-4">
